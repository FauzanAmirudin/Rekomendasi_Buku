...
if($username){
    ...
    //cek favorit
    $is_favorited = checkIfFavorited($user_id, $buku_id);
    //cek readlist
    $is_in_readlist = checkIfInreadlist($user_id, $buku_id);
}


//mengambil buku sesuai buku id
$result = mysqli_query($conn, "SELECT * FROM buku WHERE buku_id = $buku_id");
...
$komentar = mysqli_query($conn, "SELECT * FROM komentar WHERE buku_id = $buku_id");
$jumlah_komentar = mysqli_num_rows($komentar);

//mengambil genre
$genres = mysqli_query($conn, "SELECT genre_id FROM genre WHERE buku_id = $buku_id");

//rata-rata user rating
$avg_rating_result = mysqli_query($conn, "SELECT AVG(user_rating) as avg_rating FROM komentar WHERE buku_id = $buku_id");
...
// Menghitung jumlah user yang memasukkan buku ke dalam read list
$query = "SELECT COUNT(DISTINCT user_id) AS jumlah_readlist FROM readlist WHERE buku_id = $buku_id";
$resultreadList = mysqli_query($conn, $query);
$rowreadList = mysqli_fetch_assoc($resultreadList);
$jumlah_readlist = $rowreadList['jumlah_readlist'];
...
$jumlah_readlist_formatted = formatShortNumber($jumlah_readlist);
            ...
            <div class="w-8/12 flex flex-col mr-32">
                ...
                <div class="flex flex-col mr-40">
                    ...
                    <div class="ml-2 mt-3 border-b-neutral-500 border-b pb-2 flex">
                        <div class="w-2/12">Halaman</div>
                        <div class="flex">
                            <?php
                            // Menggunakan count() untuk mendapatkan total elemen dalam array
                            $totalHalaman = count($halaman);
                            // Loop melalui array penulis
                            foreach ($halaman as $index => $p):
                                // Periksa apakah ini adalah elemen terakhir dalam array
                                $isLast = ($index === $totalHalaman - 1);
                            ?>
                                <div class="w-10/12 mr-5" style="color: cornflowerblue;"><?php echo htmlspecialchars(trim($p)); ?></div>
                                <?php if (!$isLast): ?>
                                    <div class="mr-5" style="color: white;">,</div>
                    ...
                    <div class="ml-2 mt-3 border-b-neutral-500 border-b pb-2 flex">
                        ...
                        <div class="flex">
                            <?php
                            // Menggunakan count() untuk mendapatkan total elemen dalam array
                            $totalPenerbit = count($penerbit);
                            // Loop melalui array pemeran
                            foreach ($penerbit as $index => $p):
                                // Periksa apakah ini adalah elemen terakhir dalam array
                                $isLast = ($index === $totalPenerbit - 1);
                            ?>
                                <div class="w-10/12 mr-5" style="color: cornflowerblue;"><?php echo htmlspecialchars(trim($p)); ?></div>
                                <?php if (!$isLast): ?>
                                    <div class="mr-5" style="color: white;">,</div>
                ...
                <!--readlist-->
                <?php if ($username):?>
                    <form method="post" action="readlist.php">
                    <input type="hidden" name="buku_id" value="<?php echo htmlspecialchars($buku_id); ?>">
                    ...
                    <?php else: ?>
                        <button type="submit" name="add_readlist" class="flex bg-blue-500 hover:bg-blue-700 p-2 rounded-md w-5/12 mt-10 transition-colors duration-300">
